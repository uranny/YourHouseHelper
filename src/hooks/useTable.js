...
export function useTable() {
  const {
    selectedYear, setSelectedYear, selectedMonth, setSelectedMonth, records
  } = useAccountBook();
  ...
  const handleAddRecord = () => {
    ...
    records.push({ ...newRecord, amount: Number(newRecord.amount) });
  ...
  const handleEditRecord = (index, newData) => {
    ...
    if (idx !== -1) {
      records[idx] = { ...records[idx], ...newData, amount: Number(newData.amount) };
    }
  };
